...
# creat edx, validation dataset
# open previous saved files to save compile time
...
```{r description, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r plot movieid count, echo=FALSE}
edx_dat %>% 
   dplyr::count(movieId) %>% 
   ggplot(aes(n)) + 
...
```{r user movie numbers, echo-TRUE}
...
```{r print movie user title rating, echo=TRUE}
tab <- edx_dat %>% 
...
# This step takes some minutes to run at my laptop 4G windows 10
```{r}
set.seed(1)
